...
export function FeatureRow({ icon, tone = 'success', checked, children }: FeatureRowProps) {
  return (
    <div className="flex items-center gap-3 py-2">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${toneBg[tone]}`}>
        {icon || <Check className="w-5 h-5" />}
      </div>
      <span className="text-sm font-medium text-gray-900 flex-1">{children}</span>
      {checked && (
